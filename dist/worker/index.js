...
import { readWorkerState, writeWorkerState } from '../shared/state.js';
...
export class Worker {
     ...
     */
    async resume(sessionId, feedback) {
        try {
    ...
    async loadOrCreateState() {
        ...
        const sessionId = generateSessionId('worker', this.context.issue.number, this.context.emId, this.context.workerId);
        this.state = {
    ...
    async createWorkerBranch() {
        ...
        try {
            ...
            throw new Error(`Failed to create Worker branch: ${error.message}`);
        }
    }
    /**
...
Start working on this task now.`;
    ...
    async createWorkerPR() {
        ...
        try {
            ...
            throw new Error(`Failed to create Worker PR: ${error.message}`);
        }
    }
    /**
     * Build the PR body
