...
import { slugify, getDirectorBranch, getEmBranch, parseComponentFromBranch } from '../shared/branches.js';
import { readDirectorState, writeDirectorState, initConfig, writeConfig } from '../shared/state.js';
...
Provide your analysis and the JSON output below:`;
    ...
    parseEMTasks(output) {
        try {
            ...
            throw new Error(`Failed to parse EM tasks: ${error.message}`);
        }
    }
    /**
    ...
    async dispatchEMs(emTasks) {
        ...
        for (const emTask of emTasks) {
            try {
                ...
                await this.github.dispatchWorkflow('cco-em.yml', this.state.work_branch, {
                    issue_number: this.context.issue.number,
    ...
    async updateStatusComment() {
        ...
        const completedEMs = this.state.task_breakdown.filter(t => t.status === 'complete').length;
     ...
     */
    buildStatusComment(completed, total) {
...
*Last updated: ${new Date().toISOString()}*`;
        await this.github.updateIssueComment(this.context.issue.number, errorComment);
    }
    /**
    ...
    async handleEMPR(prNumber) {
        ...
        try {
            await this.github.mergePullRequest(prNumber, `Merge EM-${parsed.emId}: ${pr.title}`, pr.body);
            // Update state
            if (this.state) {
                const task = this.state.task_breakdown.find(t => t.em_id === parsed.emId);
                if (task) {
     ...
     */
    async checkAllEMsComplete() {
        if (!this.state) {
            return;
        }
        const allComplete = this.state.task_breakdown.every(t => t.status === 'complete');
        if (!allComplete) {
