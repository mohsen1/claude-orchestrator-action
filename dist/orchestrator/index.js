...
import { STATUS_LABELS, TYPE_LABELS, phaseToLabel, ORCHESTRATOR_COMMENT_MARKER, ORCHESTRATOR_REVIEW_MARKER } from '../shared/labels.js';
...
export class EventDrivenOrchestrator {
    ...
    async dispatchEvent(eventType, inputs) {
        ...
        try {
            await this.github.dispatchOrchestratorEvent(eventType, workflowInputs, {
                maxRetries: 3,
                retryDelayMs: 1000,
                idempotencyToken
            });
     ...
     */
    async runClaudeTaskWithRetry(taskFn, context) {
        ...
        throw new Error(`[${context}] Rate limit: Exhausted all ${maxRetries} retries across ${this.configManager.getConfigCount()} API configs. ` +
    ...
    async updateProgressComment(error) {
        ...
        if (ems.length > 0) {
            ...
            for (const em of ems) {
                ...
                else if (em.status === 'approved')
                    emStatusDisplay = '✅ Approved';
                else if (em.status === 'changes_requested')
                    emStatusDisplay = '📝 Changes Requested';
                else if (em.status === 'skipped')
                    emStatusDisplay = '⏭️ Skipped';
                else if (em.status === 'failed')
                    emStatusDisplay = '❌ Failed';
            ...
            for (const em of ems.filter(e => e.workers.length > 0)) {
                ...
                for (const worker of em.workers) {
                    ...
                    if (worker.status === 'merged')
                        ...
                        wStatusDisplay = '✅ Approved';
                    else if (worker.status === 'skipped')
                        wStatusDisplay = '⏭️ Skipped';
                    else if (worker.status === 'failed')
                        wStatusDisplay = '❌ Failed';
                    else
  ...
  "ems": [
    ...
    },
    {
      ...
      "must_complete_first": false
        ...
        const result = await this.runClaudeTaskWithRetry(() => this.claude.runTask(prompt, sessionId), `Director Analysis (Issue #${this.state.issue.number})`);
        if (!result.success) {
...
**REMEMBER: The "files" array is REQUIRED for every worker - do not leave it empty!**`;
     ...
     */
    processWorkerDependencies(tasks) {
        const result = [];
...
*Automated by Claude Code Orchestrator*`;
    ...
    async handlePRMerged(event) {
        ...
        if (this.state.finalPr?.number === event.prNumber) {
            ...
            try {
                ...
                console.warn('State file cleanup failed:', err.message);
            }
            await this.updateProgressComment();
            return;
        }
     ...
     */
    getOrInitReviewTrackingForPR(prNumber) {
        if (!this.state)
            return null;
        for (const em of this.state.ems) {
            for (const worker of em.workers) {
     ...
     */
    async getUnaddressedRootReviewCommentIds(prNumber) {
        const tracking = this.getOrInitReviewTrackingForPR(prNumber);
        const addressed = new Set(tracking?.addressedReviewCommentIds || []);
        ...
        for (const root of rootComments) {
            if (root.user === 'github-actions[bot]')
                continue;
            if (root.body.includes(ORCHESTRATOR_REVIEW_MARKER))
                continue;
            if (addressed.has(root.id))
                continue;
     ...
     */
    async hasCopilotCommentedReview(prNumber) {
        const reviews = await this.github.getPullRequestReviews(prNumber);
        return reviews.some(r => r.state.toLowerCase() === 'commented' &&
     ...
     */
    async isPRReadyToMerge(prNumber) {
        const reviews = await this.github.getPullRequestReviews(prNumber);
     ...
     */
    async syncPhaseForReviewIfReady() {
        if (!this.state)
            return;
        const anyWorkerRunning = this.state.ems.some(em => em.workers.some(w => w.status === 'pending' || w.status === 'in_progress'));
        const anyWorkerAwaitingReview = this.state.ems.some(em => em.workers.some(w => w.status === 'pr_created' || w.status === 'changes_requested' || w.status === 'approved'));
    ...
    async addressReview(branch, prNumber, reviewBody) {
        ...
        let addressedReviewCommentIds = [];
        let addressedIssueCommentIds = [];
        if (this.state) {
            for (const em of this.state.ems) {
                for (const worker of em.workers) {
                    if (worker.prNumber === prNumber) {
        ...
        const addressedReviewSet = new Set(addressedReviewCommentIds);
        const addressedIssueSet = new Set(addressedIssueCommentIds);
        const markReviewAddressed = (id) => {
            if (addressedReviewSet.has(id))
                return;
            addressedReviewSet.add(id);
            addressedReviewCommentIds.push(id);
        };
        const markIssueAddressed = (id) => {
            if (addressedIssueSet.has(id))
                return;
     ...
     */
    async processGeneralPRComments(prNumber, comments, _branch, opts) {
        for (const comment of comments) {
            // Skip bot comments and orchestrator-authored comments
            if (comment.user === 'github-actions[bot]')
                continue;
            if (comment.body.includes(ORCHESTRATOR_REVIEW_MARKER))
                continue;
            if (comment.body.includes('_Automated response_'))
                continue;
...
Make the changes now. Focus on what the comment is asking for.`;
                ...
                await this.github.addPullRequestComment(prNumber, `Addressed feedback from comment ${comment.id}: ${suggestedAction || 'Made the requested changes.'}\n\n${ORCHESTRATOR_REVIEW_MARKER}\n\n_Automated response_`);
                opts?.markAddressed?.(comment.id);
                console.log(`  -> Fixed and replied`);
            }
            else {
                console.log(`  -> Not actionable: ${reason}`);
                await this.github.addPullRequestComment(prNumber, `Thank you for the feedback on comment ${comment.id}. ${reason}\n\n${ORCHESTRATOR_REVIEW_MARKER}\n\n_Automated response_`);
                opts?.markAddressed?.(comment.id);
            }
        }
    }
     ...
     */
    async processInlineComments(prNumber, comments, _branch, opts) {
...
- If the feedback is just a question or doesn't require changes, do nothing`;
     ...
     */
    async handlePRsNeedingAttention() {
        if (!this.state)
            return;
        for (const em of this.state.ems) {
            for (const worker of em.workers) {
                if (worker.status !== 'pr_created' || !worker.prNumber)
                    continue;
     ...
     */
    async resolveWorkerConflicts(em, worker) {
        if (!this.state || !worker.prNumber)
            return;
        try {
            await this.github.setStatusLabel(worker.prNumber, STATUS_LABELS.ADDRESSING_FEEDBACK);
            ...
            if (!rebaseResult.success && rebaseResult.hasConflicts && rebaseResult.conflictFiles.length > 0) {
                ...
                await this.claude.resolveConflicts(sessionId, rebaseResult.conflictFiles, em.branch);
    ...
    async checkAndMergePRs() {
        ...
        for (const em of this.state.ems) {
            ...
            for (const worker of em.workers) {
                if (!worker.prNumber)
                    continue;
                if (worker.status !== 'pr_created' && worker.status !== 'approved')
                    continue;
                await this.maybeAutoMergePR(worker.prNumber);
            }
