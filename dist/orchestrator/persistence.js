 ...
 */
function mergeWorker(local, remote) {
    const merged = { ...local };
    ...
    try {
        ...
        if (isOnWorkBranch) {
            ...
            try {
                ...
                const otherChanges = stdout.split('\n').filter(line => line.trim() && !line.includes('.orchestrator/state.json'));
            ...
            if (needStash) {
                try {
                    ...
                    console.log('  Could not stash changes:', err.message);
                }
            }
            try {
                ...
                try {
                    ...
                    console.error('  Failed to switch back to original branch:', err.message);
                }
                ...
                if (needStash) {
                    try {
                        ...
                        console.log('  Could not restore stash:', err.message);
                    }
                }
            }
        }
