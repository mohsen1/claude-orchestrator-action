...
Implement this task now.`;
     ...
     */
    async waitForReviewsAndMerge(workerResults, reviewWaitMinutes) {
        ...
        const handledReviews = new Set(); // Track handled reviews to avoid duplicates
            ...
            for (const worker of successfulWorkers) {
                ...
                try {
                    ...
                    for (const review of reviews) {
                        ...
                        if (handledReviews.has(reviewKey))
                            continue;
                    ...
                    for (const comment of comments) {
                        ...
                        if (handledReviews.has(commentKey))
                            continue;
     ...
     */
    async handleReviewChangesRequested(worker, reviewBody) {
        if (!reviewBody || !worker.prNumber)
            return;
        try {
...
Please make the necessary changes to address the reviewer's feedback. Be thorough and ensure the changes align with the review comments.`;
            ...
            if (result.success) {
                ...
                const hasChanges = await GitOperations.hasUncommittedChanges();
                if (hasChanges) {
                    await GitOperations.commitAndPush(`fix: address review feedback\n\nChanges based on reviewer comments`, worker.branch);
                    console.log(`  Changes committed and pushed`);
                    // Add a comment to the PR
                    await this.github.addPullRequestComment(worker.prNumber, `I've addressed the review feedback. Please take another look.\n\n---\n*Automated response by Claude Code Orchestrator*`);
                }
                else {
     ...
     */
    async handleInlineComment(worker, comment) {
        if (!comment.body || !worker.prNumber)
            return;
        try {
            // Checkout the worker branch
            await GitOperations.checkout(worker.branch);
...
Please review the comment and make any necessary changes to the file. If the comment is a question or doesn't require code changes, just acknowledge it.`;
            ...
            if (result.success) {
                const hasChanges = await GitOperations.hasUncommittedChanges();
                if (hasChanges) {
                    await GitOperations.commitAndPush(`fix: address review comment on ${comment.path}`, worker.branch);
                    console.log(`  Changes committed`);
                }
                // Reply to the comment
                await this.github.replyToReviewComment(worker.prNumber, comment.id, `Addressed. ${hasChanges ? 'Changes pushed.' : 'No code changes needed.'}\n\n---\n*Automated response*`);
            }
        }
        catch (error) {
