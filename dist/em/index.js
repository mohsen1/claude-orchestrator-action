...
import { readEmState, writeEmState } from '../shared/state.js';
...
export class EngineeringManager {
    ...
    async loadOrCreateState() {
        ...
        const emBranch = getWorkerBranch(this.context.workBranch, this.context.emId).replace(/-w\d+$/, '');
        const sessionId = generateSessionId('em', this.context.issue.number, this.context.emId);
        this.state = {
    ...
    async createEMBranch() {
        ...
        try {
            ...
            throw new Error(`Failed to create EM branch: ${error.message}`);
        }
    }
    /**
...
Provide your analysis and the JSON output below:`;
    ...
    parseWorkerTasks(output) {
        try {
            ...
            throw new Error(`Failed to parse Worker tasks: ${error.message}`);
        }
    }
    /**
     * Dispatch Worker workflows in parallel
     */
    async dispatchWorkers(workerTasks) {
        if (!this.state) {
            throw new Error('State not initialized');
        }
        ...
        for (const workerTask of workerTasks) {
            try {
                ...
                const workerBranch = getWorkerBranch(this.state.branch, workerTask.worker_id);
                ...
                await this.github.dispatchWorkflow('cco-worker.yml', this.state.branch, {
                ...
                const worker = this.state.workers.find(w => w.worker_id === workerTask.worker_id);
                if (worker) {
    ...
    async handleWorkerPR(prNumber) {
        ...
        try {
            await this.github.mergePullRequest(prNumber, `Merge Worker-${parsed.workerId}: ${pr.title}`, pr.body);
            // Update state
            if (this.state) {
                const worker = this.state.workers.find(w => w.worker_id === parsed.workerId);
                if (worker) {
    ...
    async generateChangesSummary() {
        ...
        try {
            ...
            const summary = await this.claude.generateChangesSummary(sessionId, modifiedFiles);
...
`;
        for (const worker of this.state?.workers || []) {
            const statusEmoji = worker.status === 'complete'
                ? '✅'
                : worker.status === 'failed'
                    ? '❌'
                    : '⏳';
