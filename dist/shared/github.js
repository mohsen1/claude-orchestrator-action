...
export class GitHubClient {
    ...
    owner;
    repo;
    cachedWorkflowFilename; // Cache for auto-detected workflow name
    /**
     ...
     */
    async detectOrchestratorWorkflow() {
        try {
     ...
     * @returns void
     */
    async dispatchOrchestratorEvent(eventType, inputs, options) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (errorMessage.includes('400') ||
                    errorMessage.includes('404') ||
                    errorMessage.includes('422')) {
                    throw new Error(`Failed to dispatch ${eventType} to workflow ${this.cachedWorkflowFilename}: ${lastError.message}`);
                }
                // Retry on rate limits or transient errors
                if (attempt < maxRetries - 1) {
                    const jitter = Math.random() * 0.3;
                    const delay = baseDelayMs * Math.pow(2, attempt) * (1 + jitter);
                    console.log(`[DEBUG] Dispatch attempt ${attempt + 1} failed, retrying in ${Math.round(delay)}ms: ${lastError.message}`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                }
            }
        }
        throw new Error(`Failed to dispatch ${eventType} to workflow ${this.cachedWorkflowFilename} after ${maxRetries} attempts: ${lastError?.message}`);
    }
    /**
     * Create a Git branch
