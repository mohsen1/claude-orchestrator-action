...
async function main() {
    ...
    if (process.env.GITHUB_OUTPUT) {
        ...
        await fs.appendFile(process.env.GITHUB_OUTPUT, `stalled_count=${stalled.length}\n`);
        await fs.appendFile(process.env.GITHUB_OUTPUT, `stalled_json=${JSON.stringify(stalled)}\n`);
    }
}
main().catch(error => {
