...
export class DebugLogger {
  ...
  private mainIssueNumber?: number;
   ...
   */
  async log(event: string, details?: Record<string, unknown>, phase?: string): Promise<void> {
   ...
   * Log an error
   */
  async logError(event: string, error: Error | string, phase?: string): Promise<void> {
   ...
   */
  private async findOrCreateDebugIssue(): Promise<number> {
    const octokit = this.github.getOctokit();
    const { owner, repo } = this.getRepoInfo();
...
*Automated by Claude Code Orchestrator*`,
   ...
   */
  private formatLogEntries(): string {
    ...
    for (const entry of this.buffer) {
      ...
      if (entry.phase) body += ` [${entry.phase}]`;
      body += '\n';
   ...
   */
  getSummary(): { events: number; duration: number; errors: number } {
    return {
...
export function initDebugLogger(github: GitHubClient, enabled: boolean, mainIssueNumber?: number): DebugLogger {
...
export async function debugLog(event: string, details?: Record<string, unknown>, phase?: string): Promise<void> {
  if (globalLogger) {
    await globalLogger.log(event, details, phase);
  }
}

export async function debugLogError(event: string, error: Error | string, phase?: string): Promise<void> {
  if (globalLogger) {
